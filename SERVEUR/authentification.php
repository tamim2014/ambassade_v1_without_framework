   ...
   // Le mot de passe est-il rempli ?
    ...
    if (!empty($login)&&!empty($mdp)){
	  $sql="SELECT COUNT(*) 
			 ...
			 AND User = '" . mysqli_real_escape_string( $db , $_POST['pseudo_']) . "'
	  ...
	  $result = mysqli_fetch_row($req);
	    if ($result[0] == 0){$message= '  Identifiants incorrectes  veuillez  r&eacute;essayer! '; // a revoir
		     ...
		     //header("Location: accueil.php");// --- redirection vers la gestion des acte de naissance
		     header("Location: passport/ecritureBD.php");// --- redirection vers la gestion des passeports(ambassade des comores à Paris)
		}
	}
